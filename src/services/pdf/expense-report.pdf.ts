/**
 * Expense Report PDF — A4 tabular report of all expenses in a date range.
 * Follows the same PDFKit patterns as sales-report.pdf.ts.
 */
...
export async function buildExpenseReportPdf(data: ExpenseReportData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      size: 'A4',
      margin: 50,
      info: {
        Title: `Expense Report ${data.from} to ${data.to}`,
        Author: 'PMT by Wallx',
      },
    });
    ...
    doc.font(FONT.bold).fontSize(12).fillColor(COLORS.ink)
      .text('Expense Report', RIGHT - 130, 50, { width: 130, align: 'right' });
    doc.font(FONT.regular).fontSize(8).fillColor(COLORS.muted)
    ...
    doc.text(`Total Expenses: ${data.rows.length} entries`, LEFT, y);
    doc.text(`Total Amount: ${formatMoney(data.totalAmount)}`, LEFT + 200, y);
    y += 14;

    // Breakdown by category
    const categories = Object.entries(data.breakdown).filter(([, v]) => v.count > 0);
    ...
    // ── Table Header ────────────────────────────────────────
    const COL = { date: LEFT, desc: LEFT + 75, cat: LEFT + 320, amount: RIGHT };

    doc.font(FONT.bold).fontSize(7).fillColor(COLORS.muted);
    doc.text('DATE', COL.date, y, { width: 70 });
    doc.text('DESCRIPTION', COL.desc, y, { width: 235 });
    ...
    for (let i = 0; i < data.rows.length; i++) {
      ...
      doc.text((row.description || '—').substring(0, 60), COL.desc, y, { width: 235 });
      doc.font(FONT.regular).fontSize(7).fillColor(COLORS.muted);
      const catLabel = CATEGORY_LABELS[row.category] || row.category;
      doc.text(catLabel, COL.cat, y, { width: 95 });
    ...
    if (data.rows.length === 0) {
      doc.font(FONT.regular).fontSize(9).fillColor(COLORS.muted)
        .text('No expenses found in this period.', LEFT, y + 20, { width: PAGE_WIDTH, align: 'center' });
      y += 50;
    }
